...
class GuessNumberChatbot:
    ...
    def handle_submit(self, message, history):
        ...
        match self.state:
            ...
            case 'start':
                self.handle_game_start(message, history)

            case 'choose_gameplay':
                self.handle_choose_gameplay(message, history)
            
            case 'bot_guesses':
                self.handle_bot_guesses(message, history)

            case 'user_guesses':
                self.handle_user_guesses(message, history)
    ...
    def handle_game_start(self, message, history):
    ...
    def handle_choose_gameplay(self, message, history):
        if message in self.positive_responses:
    ...
    def handle_bot_guesses(self, message, history):
    ...
    def handle_user_guesses(self, message, history):
        if self.max_number == 0:
            if int(message) > 2:
                self.max_number = int(message)
                self.secret_number = random.randint(1, self.max_number)
